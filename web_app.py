...
if page == "🏠 Главная":
    ...
    with t_ai:
        st.markdown("<div class='glass-card'><h3>📸 Виртуальный анализ по фото</h3><p>Загрузите фото вашего блюда для примерного расчета показателей.</p></div>", unsafe_allow_html=True)
        ...
        if file_img:
            ...
            if st.button("🚀 Начать анализ ИИ"):
                with st.spinner("Нейросеть изучает состав..."):
                    time.sleep(2)
                    calories = random.randint(180, 550)
                    proteins = random.randint(10, 35)
                    fats = random.randint(5, 20)
                    carbs = random.randint(15, 65)
                    gi_val = random.randint(20, 85)
                    
                    if gi_val < 45:
                        verdict = "🟢 Безопасно. Низкий гликемический индекс."
                    elif 45 <= gi_val <= 65:
                        verdict = "🟡 Умеренно. Следите за размером порции."
                    else:
                        verdict = "🔴 Осторожно! Высокий ГИ, возможен резкий скачок сахара."

                    st.success("🤖 Примерный отчет ИИ:")
                    st.markdown(f"""
                    <div class='verdict-box'>
                        <h4 style='color: #fbbf24; margin-top:0;'>📊 Расчетные показатели (на порцию):</h4>
                        <p>🔥 <b>Калории:</b> ~{calories} ккал</p>
                        <p>🥩 <b>Белки:</b> {proteins}г | 🥑 <b>Жиры:</b> {fats}г | 🍞 <b>Углеводы:</b> {carbs}г</p>
                        <p>📈 <b>Гликемический индекс:</b> {gi_val}</p>
                        <hr style='border: 0.5px solid rgba(255,255,255,0.1)'>
                        <p><b>Вердикт:</b> {verdict}</p>
                        <p style='font-size: 11px; opacity: 0.7; font-style: italic;'>
                            *Примечание: Данные рассчитаны алгоритмом на основе визуальных признаков и носят ознакомительный характер.
                        </p>
                    </div>
                    """, unsafe_allow_html=True)
